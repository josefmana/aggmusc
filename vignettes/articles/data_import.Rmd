...
```{r classification-demo, warning=FALSE, cache=TRUE}
...
- **Demographical/clinical variables:**
  - Biological sex.
  - Age at disease onset.
  - Disease duration at the first visit.
  - First EDSS score.
- **Treatment variables:**
...
- **MRI variables:**
  ...
  - T1 blackholes volume at the closes point before years 2, 5 and 10 of the disease course.
...
```{r predictors-demo, warning=FALSE}
finished_data <- preprocess_predictors(
...
Following diagram shows completeness of the data, with patients in rows (split to patients classified as without on top or with aggressive MS on bottom) and predictors in columns, both sorted such that the most complete data sets are closer to the origin:

``` {r completeness}
plot_missingness(finished_data)
```
